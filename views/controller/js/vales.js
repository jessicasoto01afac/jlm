...
function updateadd() {
    let folio = document.getElementById('vfolio').value;
    //alert(folio);
    $.ajax({
...
function delartvalofiadd(id_valeofic) {;
    ...
    document.getElementById('del_artvoalt').value = id_valeofic
        //alert(memfi);
    let folio = id_valeofic;
    $.ajax({
...
function editarvoinf(idkardex) {;
    ...
    document.getElementById('id_voin').value = idkardex
        //alert(valofi1);
    let folio = idkardex;
    $.ajax({
        url: '../controller/php/valeofinfo.php',
...
function savecamvo() {
    ...
    //CONDICIÓN PARA SABER SI SE SURTIO O NO HAY EXISTENCIAS EN LA EDICIÓN
    if (salida > "0") {
...
function reloout() {
    ...
    let folio = document.getElementById('fvofi').innerHTML;
    var autorizar = document.getElementById('btnautorizv');
    var liberar = document.getElementById('btnliberarv');
    var surtir = document.getElementById('btnsurtirv');
    var finalizado = document.getElementById('btnfinalizv');
            ...
            if (obj.data[U].status == 'PENDIENTE') {
                ...
                $.ajax({
                    ...
                    for (D = 0; D < res.length; D++) {
                        ...
                        if (obj.data[D].status_2 == "PENDIENTE") {
                            idkardex = obj.data[D].id_kax;
                            estatus = "<button onclick='surtirvof(" + idkardex + ");' data-toggle='modal' data-target='#modal-surtirvof' type='button' title='Dar click para surtir' class='btn btn-info mg-b-10'>SURTIR</button>"
                                //acciones = "<a data-toggle='dropdown' class='btn pd-y-3 tx-gray-500 hover-info'><i class='icon ion-more'></i></a><div class='dropdown-menu dropdown-menu-right pd-10'><nav class='nav nav-style-1 flex-column'>"
                            html += "<tr><td>" + x + "</td><td>" + obj.data[D].codigo_1 + "</td><td>" + obj.data[D].descripcion_1 + "</td><td>" + obj.data[D].observa + "</td><td>" + obj.data[D].cantidad_real + "</td><td>" + obj.data[D].salida + "</td><td>" + estatus + "</td></tr>";
                        } else if (obj.data[D].status_2 == 'SURTIDO') {
                            let id_kardex = obj.data[D].id_kax;
...
function delartvoinf(idkardex) {;
    //alert(idkardex);
    document.getElementById('del_artvo').value = idkardex;
    //alert(valofi1);
    let folio = idkardex;
    $.ajax({
        url: '../controller/php/valeofinfo.php',
...
function surtirvof(idkardex) {
    //alert("sie entro");
    document.getElementById('id_surtvof').value = idkardex
        //alert(valofi1);
    let folio = idkardex;
    $.ajax({
        url: '../controller/php/valeofinfo.php',
        ...
        for (U = 0; U < res.length; U++) {
            if (obj.data[U].id_kax == id_kardex) {
                if (document.getElementById('infestavo').value == "SURTIDO" || document.getElementById('infestavo').value == "FINALIZADO") {
                    ...
                    document.getElementById('idsinexvp').value = obj.data[U].id_kax;
                    //inpus e edición
                    document.getElementById('cnsinvp').value = obj.data[U].salida;
                    document.getElementById('obdepsinvp').value = obj.data[U].observa_dep;
                    //oculta la edición
...
function saverevicionvof() {
    let refe_1 = document.getElementById('fvofi').innerHTML;
    let revision = document.getElementById('relajlvof').value;
...
function openedithsnex() {
    ...
    //document.getElementById('cnsinvp').value = 0;
...
function savedeprodu() {
    ...
    var datos = 'folio=' + folio + '&opcion=deletevale';
    //alert(datos);
    $.ajax({
        type: "POST",
        url: "../controller/php/insertvaleofi.php",
