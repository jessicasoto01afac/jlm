    ...
    $(document).ready(function() {
        ...
        $('#wizard7').steps({
            ...
            onFinished: function(event, currentIndex) {
                ...
                let folio = document.getElementById('folioprove').value; //FOLIO    
                let fecha_recl = document.getElementById('fecharepprove').value; //FECHA
                let tipo_reporte = document.getElementById('tiporeprove').value; //TIPO DE REPORTE
                let tipo_incidencia = document.getElementById('tipoincprove').value; //TIPO DE INCIDENCIA
                let orden_compra = document.getElementById('ordcproveed').value; //ORDEN DE COMPRA
                let factura = document.getElementById('factprove').value; //FACTURA
                let proveedor = document.getElementById('deprechaclie').value; //CODIGO PROVEEDOR
                let dep_report = document.getElementById('repprovedd').value; //DEPARTAMENTO REPORTA
                let pers_report = document.getElementById('repprovee').value; //PERSONA REPORTA
                let rep_jlm = document.getElementById('repcliente').innerText; //REPORTE JLM (Tomamos el texto para la base de datos )
                let code_jlm = $('#clientenote').summernote('code'); //CODIGO JLM (tomar el codigo para la base de datos)
                let date_send = document.getElementById('datesegu').value; //FECHA DE NOTIFIAICÓN
                let dept_provee = document.getElementById('departseg').value; //DEPARTAMENTO NOTIFICADO repprovedd
                let evio_a = document.getElementById('departevio').value; //ENVIO A
                let email = document.getElementById('email').value; //TELEFONO
                let telefono = document.getElementById('teldep').value; //TELEFONO
                ...
                let medio = '';
                let selectObject = document.getElementById("noticas");
                for (let i = 0; i < selectObject.options.length; i++) {
                    if (selectObject.options[i].selected == true) {
                        medio += ',' + selectObject.options[i].value;
                    }
                }
                let medios = medio.substr(1);
                //--------------------------
...
function foliovp() {
    ...
    $.ajax({
        ...
        if (respuesta == 0) {
            setTimeout("location.href = 'new_reclaprove.php';", 1500);
            //alert(respuesta);
        } else if (respuesta == 2) {
...
function addartreprove() {
    ...
    let fecha_recl = document.getElementById('fecharepprove').value;
    let remision = document.getElementById('ordcproveed').value;
    let factura = document.getElementById('factprove').value;
    let proveedor = document.getElementById('deprechaclie').value;
    ...
    if (folio == '' || fecha_recl == '' || proveedor == '' || factura == '' || remision == '') {
        ...
        $.ajax({
            ...
            if (respuesta == 0) {
                ...
                let folio2 = document.getElementById('folioprove').value; //FOLIO    
...
function openreclientes() {
    ...
    let table = $('#proveeedoreslis').DataTable({
        dom: 'Bfrtip',
        buttons: [{
                extend: 'copy',
                exportOptions: {
...
function delartaltpedart(id_delete) {;
    //alert(id_delete);
    let folio2 = document.getElementById('folioprove').value;
    $.ajax({
        url: '../controller/php/articurep.php',
        type: 'GET',
        data: 'folio=' + folio2
...
function savdelercliart() {
    ...
    let folio = document.getElementById('folioprove').value;
    ...
    $.ajax({
        ...
        for (D = 0; D < res.length; D++) {
            if (obj.data[D].folio_recl == id_reclaclient) {
                document.getElementById('folreportpr').innerHTML = obj.data[D].folio_recl;
                ...
                let area = obj.data[D].medio; //area
                // const array1 = [area];
                var data1 = area.split(',');
                //console.log(data1);
                $("#noticas").val(data1).trigger('change.select2');
...
function editararclienalt2(idreclamo) {
    //alert("entrta editar alata");
    //alert(idreclamo);
    let folio2 = document.getElementById('folioreprove').value; //FOLIO    
...
function savealtrepclie2() {
    //Datos para inserttar en al atabla de reclamoclient
    let folio = document.getElementById('folioreprove').value; //FOLIO    
    ...
    let ordencom = document.getElementById('infordencomp').value;
    let factura = document.getElementById('infpedfac').value;
    let proveedor = document.getElementById('infrepprovee').value;
...
function updatearticul2() {
    ...
    let editar = document.getElementById('openedipcliinf'); //EDITAR
    let pdfvp = document.getElementById('pdfrpclient'); //PDF
    let folio = document.getElementById('folreportpr').innerHTML; //FOLIO 
    //INFORMACIÓN 
    $.ajax({
        url: '../controller/php/inforeprovee.php',
        type: 'GET',
...
function saverepcabe() {
    ...
    let orden_compra = document.getElementById('infordencomp').value; //PEDIDO
    let factura = document.getElementById('infpedfac').value; //FACTURA
    let dep_resport = document.getElementById('deprepr').value; //DEPARTAMENTO REPORTA
    let pers_report = document.getElementById('perreprt').value; //PERSONA REPORTA
    let proveedor = document.getElementById('infrepprovee').value; //CLIENTES 
    let estatus_recl = document.getElementById('infrpestatus').value; //ESTATUS RECLAMO
    let datos = 'folio=' + folio + '&fecha_recl=' + fecha_recl + '&tipo_reporte=' + tipo_reporte + '&tipo_incidencia=' + tipo_incidencia + '&orden_compra=' + orden_compra + '&factura=' + factura + '&dep_resport=' + dep_resport + '&pers_report=' + pers_report + '&proveedor=' + proveedor + '&estatus_recl=' + estatus_recl + '&opcion=cambioheader';
    //alert(datos);
...
function editaddreport() {
    ...
    //datos para validad agregar
    let fecha_recl = document.getElementById('infrepdate').value;
    let orden_compra = document.getElementById('infordencomp').value;
    let factura = document.getElementById('infpedfac').value;
    let proveedor = document.getElementById('infrepprovee').value;
    let datos = 'folio=' + folio + '&tipo_incidencia=' + tipo_incidencia + '&tipo_reporte=' + tipo_reporte + '&cantidad=' + cantidad + '&id_articulo=' + id_articulo + '&observ_recl=' + observ_recl + '&opcion=registrar';
    //alert(datos);
...
function delartaltpedart2(id_delete) {;
    //alert(id_delete);
    let folio2 = document.getElementById('folioreprove').value;
    $.ajax({
        url: '../controller/php/articurep.php',
        type: 'GET',
        data: 'folio=' + folio2
...
function savdelercliart2() {
    ...
    let folio = document.getElementById('folioreprove').value;
    let datos = 'id_reclamo=' + id_reclamo + '&folio=' + folio + '&opcion=deleartnew';
    //alert(datos);
    $.ajax({
        type: "POST",
...
function saveupdatereport() {
    //alert("pruebas");
    let folio = document.getElementById('folioreprove').value;
    let rep_jlm = document.getElementById('clientenote').innerText;
    let code_jlm = $('#clientenote').summernote('code'); //CODIGO CLIENTE (tomar el codigo para la base de datos)
    let date_send = document.getElementById('datesegu').value;
    let dept_provee = document.getElementById('departseg').value;
    let evio_a = document.getElementById('departevio').value;
    let telefono = document.getElementById('teldep').value;
    let email = document.getElementById('email').value;
    let seguimiento = document.getElementById('jlmnote').innerText; //REPORTE JLM (Tomamos el texto para la base de datos )
    let code_seguimiento = $('#jlmnote').summernote('code'); //CODIGO JLM (tomar el codigo para la base de datos)
    let conclusion = document.getElementById('conclicionnote').innerText; //REPORTE conclusion (Tomamos el texto para la base de datos )
    let medio = '';
    let selectObject = document.getElementById("noticas");
    for (let i = 0; i < selectObject.options.length; i++) {
        if (selectObject.options[i].selected == true) {
    ...
    let medios = medio.substr(1);
    let code_conclucion = $('#conclicionnote').summernote('code'); //CODIGO conclusion (tomar el codigo para la base de datos)
    let datos = 'folio=' + folio + '&rep_jlm=' + rep_jlm + '&code_jlm=' + code_jlm + '&date_send=' + date_send + '&dept_provee=' + dept_provee + '&evio_a=' + evio_a + '&telefono=' + telefono + '&email=' + email + '&seguimiento=' + seguimiento + '&code_seguimiento=' + code_seguimiento + '&conclusion=' + conclusion + '&code_conclucion=' + code_conclucion + '&medios=' + medios + '&opcion=updatesavereport';
    //alert(datos);
    if (folio == '') {
        Swal.fire({
            type: 'info',
...
function finalizarep() {
    ...
    let folio = document.getElementById('folioreprove').value; //FOLIO DEL MEMO
...
function pdfvp() {
    var folio = document.getElementById('folioreprove').innerHTML;
    //alert(folio);
