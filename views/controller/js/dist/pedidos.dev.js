...
function editpediinf() {
  ...
  document.getElementById('infdirec').disabled = false; //muestra los botones

  document.getElementById('addarticp').style.display = "";
  document.getElementById('cancelpe').style.display = ""; //muestra el boton de cerrar editar
...
function closedithpin() {
  ...
  document.getElementById('infdirec').disabled = true; //muestra los botones
    ...
    $.ajax({
      ...
      for (D = 0; D < res.length; D++) {
        if (obj.data[D].id_kax == id_arped) {
          ...
          obj.data[D].costo + '*' + obj.data[D].artubicac + '*' + obj.data[D].descripcion_1;
          var o = datos.split("*");
