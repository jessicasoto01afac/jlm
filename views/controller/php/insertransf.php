...
//CONDICIONES------------------------------------------------------------------------------
    ...
    if($opcion === 'registrar'){
        ...
        //if (comprobacion ($id_articulo_final,$id_extendido,$id_etiquetas,$conexion)){
            if (comprobacion ($id_articulo_final,$conexion)){
            /*$hojas = $_POST['hojas'];
            ...
            if (registrar($id_articulo_final,$conexion)){
                echo "0";
               // $usuario='pruebas';
                historial($usuario,$id_articulo_final,$conexion);
            }else{
                echo "1";
            }
        }else{
    ...
    //Condición donde elimina usuario 
        ...
        if (comprobcolors ($final,$extendido,$conexion)){
            ...
            $divicion = $_POST['divicion'];
            $tipo = $_POST['tipo'];
            if (regisartfoext($final,$extendido,$multiplic,$divicion,$tipo,$conexion)){
                echo "0";
               // $usuario='pruebas';
            }else{
                echo "1";
        ...
        if (comprobcolors ($final,$extendido,$conexion)){
            ...
            $divicion = $_POST['divicion'];
            $tipo = $_POST['tipo'];
            if (regisartfoext2($final,$extendido,$multiplic,$divicion,$tipo,$conexion)){
                echo "0";
               // $usuario='pruebas';
            }else{
                echo "1";
...
function comprobacion ($id_articulo_final,$conexion){
    $query="SELECT * FROM transforma WHERE id_articulo_final = '$id_articulo_final' AND estado = 8";
    $resultado= mysqli_query($conexion,$query);
    if($resultado->num_rows==0){
        return true;
    }else{
...
function comprobcolors ($final,$extendido,$conexion){
    $query="SELECT * FROM transformation WHERE id_articfial = '$final' AND id_articulo = '$extendido' AND estado = 0";
    $resultado= mysqli_query($conexion,$query);
    if($resultado->num_rows==0){
        return true;
    }else{
...
function registrar ($id_articulo_final,$conexion){
    $query="UPDATE transformation SET estado=0 where id_articfial='$id_articulo_final' and estado=2";
    if(mysqli_query($conexion,$query)){
        return true;
    }else{
        return false;
...
/*function regisartfoext ($final,$extendido,$multiplic,$divicion,$conexion){
...
function regisartfoext ($final,$extendido,$multiplic,$divicion,$tipo,$conexion){
    $query="INSERT INTO transformation (id_trans,id_articfial,id_articulo,type_art,hojas,division,estado) VALUES(0,'$final','$extendido','$tipo','$multiplic',$divicion,2)";
    if(mysqli_query($conexion,$query)){
        return true;
    }else{
        return false;
...
function regisartfoext2 ($final,$extendido,$multiplic,$divicion,$tipo,$conexion){
    $query="INSERT INTO transformation (id_trans,id_articfial,id_articulo,type_art,hojas,division,estado) VALUES(0,'$final','$extendido','$tipo','$multiplic',$divicion,0)";
    if(mysqli_query($conexion,$query)){
        return true;
    }else{
        return false;
...
function eliminar ($id_transformacion,$conexion){
    $query="UPDATE transformation SET estado='1' WHERE id_articfial = '$id_transformacion'";
    if(mysqli_query($conexion,$query)){
        return true;
    }else{
        return false;
...
function eliminarcolos ($idtrans,$id_colorss,$conexion){
    $query="UPDATE transformation SET estado='1' WHERE id_trans = '$id_colorss'";
    if(mysqli_query($conexion,$query)){
        return true;
    }else{
        return false;
...
function historial($usuario,$id_articulo_final,$conexion){
    ini_set('date.timezone','America/Mexico_City');
    $fecha = date('Y').'/'.date('m').'/'.date('d').' '.date('H:i:s'); //fecha de realización
    $query = "INSERT INTO historial VALUES (0,'$usuario','AGREGA UN NUEVO ARTICULO DE TRANSFORMACION',' ARTICULO FINAL: ' '$id_articulo_final','$fecha')";
    if(mysqli_query($conexion,$query)){
        return true;
    }else{
        return false;
